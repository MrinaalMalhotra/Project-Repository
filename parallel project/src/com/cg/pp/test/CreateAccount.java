...
public class CreateAccount {
	...
	public void testwrounginput() throws AccountException {
		AccountService aserv=new AccountServiceImpl();
		Person p=new Person("Twinkle",23,"1345724698373","9999707207","twinkle@gmail.com");
		Account acc=new Account(p,0);
		if(aserv.validateDetails(p)){
			int b=aserv.createAccount(acc);
			assertEquals(acc.getNumber(),b);
	...
	public void testWronginput2() throws AccountException {
		AccountService aserv=new AccountServiceImpl();
		Person p=new Person("Twinkle",23,"1345724698373583","999970","twinkle@gmail.com");
		Account acc=new Account(p,0);
		if(aserv.validateDetails(p)){
			int b=aserv.createAccount(acc);
			assertEquals(acc.getNumber(),b);
	...
	public void testWrounginput3() throws AccountException {
		AccountService aserv=new AccountServiceImpl();
		Person p=new Person("Twinkle",23,"1345724698373583","9999707207","twinkle");
		Account acc=new Account(p,0);
		if(aserv.validateDetails(p)){
			int b=aserv.createAccount(acc);
			assertEquals(acc.getNumber(),b);
